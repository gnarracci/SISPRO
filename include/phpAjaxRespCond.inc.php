...
include('mainFuncRespCond.inc.php');
...
if($errorDbConexion == false){
	...
	if(isset($_POST) && !empty($_POST)){
		...
		switch ($_POST['accion']) {
			case 'addRespuesta':
				// Armamos el query
				$query = sprintf("INSERT INTO resp_condicion
								 SET resp_condicion='%s', id_asociado='%d', pctaje_minimo='%d'",
								 $_POST['resp_condicion'],$_POST['id_asociado'],$_POST['pctaje_minimo']);
				...
				$id_resp_condicionOK = $mysqli -> insert_id;
				...
				if($resultadoQuery == true){
					...
					$contenidoOK = '
						<tr>
							...
							<td class="centerTXT"><a class="btn btn-small" href="'.$id_resp_condicionOK.'">Editar</a></td>
						<tr>
					';
			...
			case 'editRespuesta':
				// Armamos el query
				$query = sprintf("UPDATE resp_condicion
								 SET resp_condicion='%s', id_asociado='%d', pctaje_minimo='%d'
								 WHERE id_resp_condicion=%d LIMIT 1",
								 $_POST['resp_condicion'],$_POST['id_asociado'],$_POST['pctaje_minimo'],$_POST['id_resp_condicion']);
