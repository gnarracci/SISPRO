...
//Funcion para extraer el listado de Preguntas
    function consultaPreguntas($linkDB){
        ...
        $consulta = $linkDB -> query("SELECT id_rapport,puesto_trabajo,pregunta_rapport,respuesta_rapport,pregunta_pctaje FROM cuestionario_rapport ORDER BY puesto_trabajo ASC"); 
            ...
            while($listadoOK = $consulta -> fetch_assoc())
            {
                $salida .= '
                    <tr>
                        ...
                        <td>'.$listadoOK['pregunta_rapport'].'</td>
                        <td>'.$listadoOK['respuesta_rapport'].'</td>
                        <td style="font-weight: bold;text-align:center;">'.$listadoOK['pregunta_pctaje'].'</td>
                        <td id="edit" class="centerTXT"><a class="btn btn-small" href="'.$listadoOK['id_rapport'].'">Editar</a></td>
                    </tr>
                ';   
            }
        }
