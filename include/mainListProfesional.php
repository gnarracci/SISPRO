...
//Funcion para extraer el listado de Preguntas
    function consultaPreguntas($linkDB){
        ...
        $consulta = $linkDB -> query("SELECT id_profesional,puesto_trabajo,pregunta_profesional,respuesta_profesional,pregunta_pctaje FROM cuestionario_profesional ORDER BY id_profesional ASC"); 
        ...
        if($consulta -> num_rows != 0){
            ...
            while($listadoOK = $consulta -> fetch_assoc())
            {
                $salida .= '
                    <tr>
                        <td style="color: red;font-weight: bold;text-align:center;">'.$listadoOK['id_profesional'].'</td>
