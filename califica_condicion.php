...
<!DOCTYPE html>
	<html lang="es">
		<head>
			...
			<title>Calificación Preguntas de Condición</title>
			<link rel="shortcut icon" href="favicon.ico"/>
			<link rel="stylesheet" href="califica_condicion.css"/>
				...
				$(function(){
					$('#dialog').dialog({
						...
						buttons: [
						{
							text: "OK",
							click: function(){
								$(location).attr('href','examen_profesional.php');
							}
						},
						{
							text: "Cancelar",
		...
		<body>
			...
			<section>
				...
				<div id="cuerpo">
					...
					$numPregs = 10; //Numero maximo de Preguntas por Item a Evaluar
					    ...
					    for ($cju=0;$cju<count($preg);$cju++){
					        $consulta = mysql_query("SELECT id_resp_condicion FROM corresponde_condicion WHERE id_condicion = '$preg[$cju]' AND sipi = '1'",$conexionsspi);
					        $idres = mysql_result($consulta,0,'id_resp_condicion');
					        $consulta = mysql_query("SELECT resp_condicion FROM resp_condicion WHERE id_resp_condicion = '$idres'",$conexionsspi);
					        $respuestidirijilla = mysql_result($consulta,0,'resp_condicion');
					        if ($respDada[$cju] == $respuestidirijilla) $calif+=1;
					    }
					    
					    //Actualizacion de los Registros de Calificación y Presentación de la Prueba
					    $actu = mysql_query("UPDATE usuarios_encuesta SET calif_condicion = '$calif' WHERE cedula = '$id' AND id_eval = '$identificador'",$conexionsspi);
				...
				<a href="examen_profesional.php"><img src="imagenes/continuar-2.png" width="151px" height="77px" alt="continuar" id="continuar" title="Continuar con las Preguntas de Formación" /></a>
