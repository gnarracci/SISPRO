...
<!DOCTYPE html>
	<html lang="es">
		<head>
			...
			<title>Encuesta de Evaluación para Condición</title>
			<link rel="shortcut icon" href="favicon.ico"/>
			<link rel="stylesheet" href="examen_condicion.css"/>
		...
		<body>
			...
			<header>
				...
				<img src="imagenes/eval_condicion.png" width="960px" height="204px" alt="membrete" id="eval_header" />
			...
			<div id="div_session_timeout">
				...
				<script>
					...
					var url = 'examen_profesional.php'; // Pagina para la Redireccion
			...
			<section id="identificador">
				...
				<span id="id">Encuesta de Evaluación de Preguntas de Condición</span>
			...
			<section id="test">
				...
				<p id="notice">El siguiente grupo de Preguntas medirán de manera General sus Condiciones Personales</p>
				...
				<div id="cuerpo">
					...
					echo '<form action="califica_condicion.php" method="post">'."\n";
					for ($r=0;$r<$numPregs;$r++){
						$pregActu = mysql_query("SELECT pregunta_condicion FROM cuestionario_condicion WHERE id_condicion = '$vector[$r]'",$conexionsspi);

						$txtPregActu = mysql_result($pregActu,0,'pregunta_condicion');

						$tpPreg = mysql_query("SELECT respuesta_condicion FROM cuestionario_condicion WHERE id_condicion = '$vector[$r]'",$conexionsspi);

						$tipoPreg = mysql_result($tpPreg,0,'respuesta_condicion');
						...
						if ($tipoPreg == 2) { //Abierta
							...
							$consOps = mysql_query("SELECT id_resp_condicion FROM corresponde_condicion WHERE id_condicion = '$vector[$r]'",$conexionsspi);
							...
							for ($hui=0;$hui<$numResp;$hui++){
								...
								$consResp = mysql_query("SELECT resp_condicion from resp_condicion where id_resp_condicion='".mysql_result($consOps,$hui,'id_resp_condicion')."'",$conexionsspi);

								$valor = mysql_result($consResp,0,'resp_condicion');
					...
					echo '<input type="submit" class="btn btn" id="boton" value="Calificar Condición">'."\n";
				...
				<p id="notice-2">Recuerde verficar sus respuestas antes de presionar el botón de "Calificar Condición"</p>				
