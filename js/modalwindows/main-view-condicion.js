$(function(){
		...
		$('#agregarRespuesta').dialog({
			title: 'Agregar Respuesta de Condición',
			autoOpen: true
			});
		});
		...
		$('#agregar').click(function(){
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				url: "include/phpAjaxRespCond.inc.php",
				data: string,
		...
		$('table').on('click','#listaRespuestasOK a', function (event){
			...
			//Id Respuesta
			$('#id_resp_condicion').val($(this).attr('href'));
			...
			$('#resp_condicion').val($(this).parent().parent().children('td:eq(0)').text());
			...
			//Abrimos el Formulario
			$('#agregarRespuesta').dialog({
				title: 'Editar Respuesta de Condición',
				autoOpen: true
			});
		});	
	});