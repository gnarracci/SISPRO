$(function(){
		...
		$('#save').on('click',function(){
			...
			$('#agregarPerfil').dialog({
			title: 'Agregar Pregunta de Rapport',
			autoOpen: true
			});
		});
		...
		$('#agregar').click(function(){
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				url: "include/phpAjaxQuestions.inc.php",
				data: string,
	...
	$('table').on('click','#listaPreguntasOK a', function (event){
		...
		$('#pregunta_rapport').val($(this).parent().parent().children('td:eq(1)').text());

		//Seleccionando Tipo de Respuesta
		$('#respuesta_rapport').val($(this).parent().parent().children('td:eq(2)').text());
		...
		//Abrimos el Formulario
		$('#agregarPerfil').dialog({
			title: 'Editar Pregunta de Rapport',
			autoOpen: true
		});
	});
});