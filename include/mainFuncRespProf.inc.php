...
//Funcion para extraer el listado de Preguntas
    function consultaRespuestas($linkDB){
        ...
        $consulta = $linkDB -> query("SELECT id_resp_profesional,resp_profesional,id_asociado,pctaje_minimo FROM resp_profesional ORDER BY id_resp_profesional ASC"); 
        ...
        if($consulta -> num_rows != 0){
            ...
            while($listadoOK = $consulta -> fetch_assoc())
            {
                $salida .= '
                    <tr>
                        <td>'.$listadoOK['resp_profesional'].'</td>
                        ...
                        <td id="edit" class="centerTXT"><a class="btn btn-small" href="'.$listadoOK['id_resp_profesional'].'">Editar</a></td>
                    </tr>
                ';   
            }
        }
