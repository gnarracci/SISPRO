...
//Funcion para extraer el listado de Preguntas
    function consultaPreguntas($linkDB){
        ...
        $consulta = $linkDB -> query("SELECT id_condicion,puesto_trabajo,pregunta_condicion,respuesta_condicion,pregunta_pctaje FROM cuestionario_condicion ORDER BY puesto_trabajo ASC");
            ...
            while($listadoOK = $consulta -> fetch_assoc())
            {
                $salida .= '
                    <tr>
                        ...
                        <td>'.$listadoOK['pregunta_condicion'].'</td>
                        <td>'.$listadoOK['respuesta_condicion'].'</td>
                        <td style="font-weight: bold;text-align:center;">'.$listadoOK['pregunta_pctaje'].'</td>
                        <td id="edit" class="centerTXT"><a class="btn btn-small" href="'.$listadoOK['id_condicion'].'">Editar</a></td>
                    </tr>
                ';   
            }
        }
