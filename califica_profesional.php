...
<!DOCTYPE html>
	<html lang="es">
		<head>
			...
			<title>Calificación Preguntas de Perfil Profesional</title>
			<link rel="shortcut icon" href="favicon.ico"/>
			<link rel="stylesheet" href="califica_profesional.css"/>
				...
				$(function(){
					$('#dialog').dialog({
						...
						buttons: [
						{
							text: "OK",
							click: function(){
								$(location).attr('href','totalizador.php');
							}
						},
						{
							text: "Cancelar",
				...
				<div id="cuerpo">
					...
					$numPregs = 10; //Numero maximo de Preguntas por Item a Evaluar
					    ...
					    for ($cju=0;$cju<count($preg);$cju++){
					        $consulta = mysql_query("SELECT id_resp_profesional FROM corresponde_profesional WHERE id_profesional = '$preg[$cju]' AND sipi = '1'",$conexionsspi);
					        $idres = mysql_result($consulta,0,'id_resp_profesional');
					        $consulta = mysql_query("SELECT resp_profesional FROM resp_profesional WHERE id_resp_profesional = '$idres'",$conexionsspi);
					        $respuestidirijilla = mysql_result($consulta,0,'resp_profesional');
					        if ($respDada[$cju] == $respuestidirijilla) $calif+=1;
					    }
					    
					    //Actualizacion de los Registros de Calificación y Presentación de la Prueba
					    $actu = mysql_query("UPDATE usuarios_encuesta SET calif_profesional = '$calif' WHERE cedula = '$id' AND id_eval = '$identificador'",$conexionsspi);
