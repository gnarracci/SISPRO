$(function(){
		...
		$('#agregarRespuesta').dialog({
			title: 'Agregar Respuesta Profesional',
			autoOpen: true
			});
		});
		...
		$('#agregar').click(function(){
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				url: "include/phpAjaxRespProf.inc.php",
				data: string,
		...
		$('table').on('click','#listaRespuestasOK a', function (event){
			...
			//Id Respuesta
			$('#id_resp_profesional').val($(this).attr('href'));
			...
			$('#resp_profesional').val($(this).parent().parent().children('td:eq(0)').text());
			...
			//Abrimos el Formulario
			$('#agregarRespuesta').dialog({
				title: 'Editar Respuesta Profesional',
				autoOpen: true
			});
		});	
	});