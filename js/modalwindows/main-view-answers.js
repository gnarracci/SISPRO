	$(function(){
		...
		$('#agregarRespuesta').dialog({
			title: 'Agregar Respuesta de Rapport',
			autoOpen: true
			});
		});
		...
		$('#agregar').click(function(){
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				url: "include/phpAjaxRespRapp.inc.php",
				data: string,
		...
		$('table').on('click','#listaRespuestasOK a', function (event){
			...
			//Id Respuesta
			$('#id_resp_rapport').val($(this).attr('href'));
			...
			$('#resp_rapport').val($(this).parent().parent().children('td:eq(0)').text());
			...
			//Abrimos el Formulario
			$('#agregarRespuesta').dialog({
				title: 'Editar Respuesta de Rapport',
				autoOpen: true
			});
		});	
	});