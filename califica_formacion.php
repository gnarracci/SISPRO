...
<!DOCTYPE html>
	<html lang="es">
		<head>
			...
			<title>Calificación Preguntas de Formación</title>
			<link rel="shortcut icon" href="favicon.ico"/>
			<link rel="stylesheet" href="califica_formacion.css"/>
				...
				$(function(){
					$('#dialog').dialog({
						...
						buttons: [
						{
							text: "OK",
							click: function(){
								$(location).attr('href','examen_condicion.php');
							}
						},
						{
							text: "Cancelar",
		...
		<body>
			...
			<section>
				...
				<div id="cuerpo">
					...
					$numPregs = 10; //Numero maximo de Preguntas por Item a Evaluar
					    ...
					    for ($cju=0;$cju<count($preg);$cju++){
					        $consulta = mysql_query("SELECT id_resp_formacion FROM corresponde_formacion WHERE id_formacion = '$preg[$cju]' AND sipi = '1'",$conexionsspi);
					        $idres = mysql_result($consulta,0,'id_resp_formacion');
					        $consulta = mysql_query("SELECT resp_formacion FROM resp_formacion WHERE id_resp_formacion = '$idres'",$conexionsspi);
					        $respuestidirijilla = mysql_result($consulta,0,'resp_formacion');
					        if ($respDada[$cju] == $respuestidirijilla) $calif+=1;
					    }
					    
					    //Actualizacion de los Registros de Calificación y Presentación de la Prueba
					    $actuf = mysql_query("UPDATE usuarios_encuesta SET calif_formacion = '$calif' WHERE cedula = '$id' AND id_eval = '$identificador'",$conexionsspi);
				...
				<a href="examen_condicion.php"><img src="imagenes/continuar-1.png" width="151px" height="77px" alt="continuar" id="continuar" title="Continuar con las Preguntas de Formación" /></a>
