$(function(){
		...
		$('#save').on('click',function(){
			...
			$('#agregarPerfil').dialog({
			title: 'Agregar Pregunta de Condición',
			autoOpen: true
			});
		});
		...
		$('#agregar').click(function(){
			...
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				url: "include/phpAjaxQuestionsCon.inc.php",
				data: string,
	...
	$('table').on('click','#listaPreguntasOK a', function (event){
		...
		//Id Pregunta
		$('#id_condicion').val($(this).attr('href'));
		...
		$('#pregunta_condicion').val($(this).parent().parent().children('td:eq(1)').text());

		//Seleccionando Tipo de Respuesta
		$('#respuesta_condicion').val($(this).parent().parent().children('td:eq(2)').text());
		...
		//Abrimos el Formulario
		$('#agregarPerfil').dialog({
			title: 'Editar Pregunta de Condición',
			autoOpen: true
		});
	});
});